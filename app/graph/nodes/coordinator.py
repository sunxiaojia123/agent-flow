...
- 如果是交易相关需求(采购钢材、询价、查库存、物流查询、供应商对比等) → 只回复 handoff_to_planner，不要做其他解释
- 如果用户追问某个供应商的详情 → 只回复 handoff_to_planner
- 如果用户说"选这家"、"就第一家的"等选商意图 → 只回复 handoff_to_planner

## 重要
对于交易需求，你只需要回复 "handoff_to_planner" 这几个字，让规划员来处理。"""
...
def create_coordinator_node():
    ...
    def coordinator_node(state: AgentState) -> Command:
        ...
        if "handoff_to_planner" in response.content:
            return Command(goto="planner", update={"coordinator_reply": None})
